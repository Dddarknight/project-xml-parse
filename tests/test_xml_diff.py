...
def make_set_from_lines(result):
    ...
    for line in result:
        ...
        required_set.add(line_corrected)
    return required_set
...
def test_xml_diff():
    ...
    xml_difference(
        get_fixture_path('fixture1.xml'),
        get_fixture_path('fixture2.xml'),
        file_path)
    with open(get_fixture_path('result_min.json')) as expected:
        with open(file_path) as read_file:
            assert make_set_from_lines(
                expected.read()) == make_set_from_lines(read_file.read())
