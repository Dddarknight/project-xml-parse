def make_diff_tree(node1, node2):
    ...
    if return_itinerary in removed:
        ...
        tree.update({'Pricing': "The price can't be compared"
                                " because of the absence "
                                "of return itinerary"})
        common_keys.remove('Pricing')
    if return_itinerary in added:
        added.remove(return_itinerary)
        tree.update({return_itinerary: (f"added {node2[return_itinerary]} "
                                        f"The price can't be compared")})
        answer = ("The price can't be compared because"
                  " of the absence of onward itinerary")
        tree.update({'Pricing': answer})
        common_keys.remove('Pricing')
